...
let
  ...
  targetPlatform = if platform.isLinux then "linux" else "darwin";
  targetArch = if platform.isAarch64 then "arm64" else "x64";
  # For x64, use baseline variant for wider CPU compatibility
  nodeFileName = "pi_natives.${targetPlatform}-${targetArch}${if targetArch == "x64" then "-baseline" else ""}.node";
  # bun compile target (e.g., bun-linux-x64-modern)
  bunTarget = "bun-${targetPlatform}-${targetArch}${if targetArch == "x64" then "-modern" else ""}";
